...
# get inputs
cols1 = st.columns(4)
LoadingFlow = round(float(cols1[0].number_input('Loading_flow(進料流量)', 79., 90., 83.972,step=0.1, format="%.3f")), 3)
MaterialNA = round(float(cols1[1].number_input('Material_NA(原料雜質)', 15., 21., 16.704,step=0.1, format="%.3f")), 3)
SteamFlow= round(float(cols1[2].number_input('Steam_flow(蒸汽用量)', 30., 36., 33.197, step=0.1, format="%.3f")), 3)
BackwashFlow = round(float(cols1[3].number_input('Backwash_flow(逆洗量)', 43., 57., 50.938, step=0.1, format="%.3f")), 3)
cols2 = st.columns(4)
SolventFlow = round(float(cols2[0].number_input('Solvent_flow(溶劑流量)', 27., 70., 42.12, step=0.1, format="%.3f")), 3)
RaffinateA = round(float(cols2[1].number_input('Raffinate_A(萃餘油濃度)', 1., 1.9, 1.595, step=0.1, format="%.3f")), 3)
HeatExchangerTemp = (float(cols2[2].number_input('HeatExchanger_temp(熱交換器溫度)', 87., 102., 89.988, step=0.1, format="%.3f")))
TowerTopPres = round(float(cols2[3].number_input('TowerTop_pres(塔頂壓力)', 1.1, 1.8, 1.464, step=0.1, format="%.3f")), 3)
